...
if (strlen($_SESSION['vpmsaid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $enteringtime = $_POST['fromdate'];
        // $goingtime = $_POST['todate'];

        $chargePerSlot = 0;
        $sql = "SELECT * FROM `garages` WHERE `garageName` LIKE '$gname'";
        $retval = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_array($retval, MYSQLI_ASSOC)) {
            $chargePerSlot = $row['costperslot'];
            
        }

        $parkingCharge = (strtotime($goingtime) - strtotime($enteringtime))/(600*6)*$chargePerSlot;

        // $query = mysqli_query($con, "INSERT INTO `tblvehicle`(`ParkingNumber`, `VehicleCategory`, `garageName`, `VehicleCompanyname`, `RegistrationNumber`, `OwnerName`, `OwnerContactNumber`, `InTime`, `OutTime`,`ParkingCharge`) VALUES ('$parkingnumber','$catename','$gname','$vehcomp','$vehreno','$ownername','$ownercontno','$enteringtime','$goingtime','$parkingCharge')");
        $query = mysqli_query($con, "INSERT INTO `tblvehicle`(`ParkingNumber`, `VehicleCategory`, `garageName`, `VehicleCompanyname`, `RegistrationNumber`, `OwnerName`, `OwnerContactNumber`, `InTime`,`ParkingCharge`) VALUES ('$parkingnumber','$catename','$gname','$vehcomp','$vehreno','$ownername','$ownercontno','$enteringtime','$parkingCharge')");
        if ($query) {
                            ...
                            <div class="card-body card-block">
                                <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                                    ...
                                    <div class="row form-group">
                                        <div class="col col-md-3"><label for="select" class=" form-control-label">Select</label></div>
                                        <div class="col-12 col-md-9">
                                            <select name="GarageName" id="GarageName" class="form-control">
                                                <option value="0">Select Garage</option>
                                                <?php $query = mysqli_query($con, "select * from availableslots");
                                                while ($row = mysqli_fetch_array($query)) {
                                                ?>
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <div class="col-12 col-md-9"><input type="datetime-local" name="fromdate" id="fromdate" class="form-control" required="true"></div>
                                    <!-- </div>
                                    <div class="row form-group">
                                        <div class="col col-md-3"><label for="text-input" class=" form-control-label">To Date</label></div>
                                        <div class="col-12 col-md-9"><input type="datetime-local" name="todate"  class="form-control" required="true"></div>
                                    </div> -->
