...
if (strlen($_SESSION['vpmsaid'] == 0)) {
  ...
  if (isset($_POST['submit'])) {
    ...
    $parkingCharge = (strtotime($exitTime) - strtotime($inTime)) / (600 * 6) * $chargePerSlot;
    // $parkingCharge = $chargePerSlot;
    // echo $parkingCharge;

    $query = mysqli_query($con, " UPDATE `tblvehicle` SET `ParkingCharge`=$parkingCharge WHERE ID='$cid'");
    $query = mysqli_query($con, "update  tblvehicle set Remark='$remark',Status='$status',OutTime = '$exitTime' where ID='$cid'");
    if ($query) {
